...
- [Foobar2000](https://www.foobar2000.org/windows) and the [vgmstream plugin](https://www.foobar2000.org/components/view/foo_input_vgmstream) - Used to open and rip audio that are in the `.fsb` format.
...
- [C2 Arcade Texture Gamma Fix](https://github.com/MythicalBry/cars-2-arcade-texture-gamma-fix-io/releases) - To be used for custom textures targeting Cars 2: Arcade. Textures that don't have the `_n` suffix need to be patched in order for them to appear gamma correct in Cars 2: Arcade.
- [unluac](https://github.com/x3zvawq/unluac-rs/releases) - Used to decompile files created by Luac. Most files will have the `.lua` extension, but there may be some exceptions. There is a web version of unluac, providing for easier access. Visit that [here](https://unluac.x3zvawq.com/) if you have trouble with the `.exe`.