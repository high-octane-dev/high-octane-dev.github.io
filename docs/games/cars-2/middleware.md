...
The FMOD API refers to all DLL files contained within the root folder of the game that start with `fmod_` alongside both `fmodex.dll` and `fmodexL.dll`. The versions of these DLL files are different between the two PC versions of the game. Arcade uses the `4.32.20` versions of the DLL files while Win32Wii uses `4.32.8`.
